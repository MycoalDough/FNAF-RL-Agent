...
class QTrainer:
    ...
    def train_step(self, state, action, reward, next_state, done, priorities):
        ...
        num_classes = 16
        action_one_hot = torch.nn.functional.one_hot(action.long(), num_classes=num_classes)
        q_state_values = torch.sum(action_one_hot * q_values, dim=1, keepdim=True)

        print(q_state_values.shape)
        print(target_q_values.shape)
        error = q_state_values - target_q_values
        priorities_tensor = torch.tensor(priorities, dtype=torch.float32).view(-1, 1)
        loss = (priorities_tensor * error.pow(2)).mean()
